...
export async function PUT(
  ...
  try {
    ...
    const { courseId, resourceId } = resolvedParams;
    ...
    const resource = await prisma.resource.update({
      where: { id: resourceId, courseId: courseId, classId: null }, // Asegurarse de que sea un recurso general
      data: { nombre, url, tipo, filePath, mimeType, fileSize },
    });

    return NextResponse.json(resource);
  } catch (error) {
    console.error('[PROFESOR_COURSE_RESOURCE_ID_PUT]', error);
...
export async function DELETE(
  ...
  try {
    ...
    console.error('[PROFESOR_COURSE_RESOURCE_ID_DELETE]', error);
    return new NextResponse('Internal Error', { status: 500 });
  }
}
