...
export default function CourseResourcesPage() {
  ...
  const fetchResources = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/profesor/courses/${courseId}/resources`);
      if (!res.ok) {
  ...
  const handleAddResource = async (e: React.FormEvent) => {
    ...
    if (!newResourceUrl.trim() && !newSelectedFile) {
  ...
  const handleEditResource = async (resourceId: string) => {
    ...
    try {
      const res = await fetch(`/api/profesor/courses/${courseId}/resources/${resourceId}`, {
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(resourceData),
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6 text-gray-900">
        Gestión de Recursos Generales para el Curso: {courseTitle}
      </h1>
          ...
          <div className="border-t pt-4 mt-4">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value) setNewSelectedFile(null); // Clear file if URL is entered
                }}
                className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
            ...
            <div>
              ...
              <input
                ...
                id="newResourceFile"
                onChange={(e) => {
                  setNewSelectedFile(e.target.files ? e.target.files[0] : null);
              ...
              />
              {newSelectedFile && <p className="text-gray-600 text-sm mt-1">Archivo seleccionado: {newSelectedFile.name}</p>}
            </div>
          </div>
      ...
      <div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        ...
        {resources.length === 0 ? (
          <p>No hay recursos creados para este curso aún.</p>
        ) : (
          <ul>
