...
import Link from 'next/link'; // Añadir esta línea
...
export default function EditCoursePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (session?.user?.isSuperAdmin) {
        const instructorsRes = await fetch('/api/admin/profesores');
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mb-6 text-gray-900">Editar Curso: {courseTitle}</h1>
